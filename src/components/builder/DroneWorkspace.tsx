...
const Propeller = ({ position, spinning = true }: { position: [number, number, number]; spinning?: boolean }) => {
...
// 3D Frame Component
const DroneFrame = ({ hasFrame }: { hasFrame: boolean }) => {
...
const Battery = ({ position }: { position: [number, number, number] }) => (
...
// Main Drone Model
const DroneModel = ({ components }: { components: PlacedComponent[] }) => {
  ...
  return (
    <Float speed={1.5} rotationIntensity={0.1} floatIntensity={0.3}>
      <group>
        <DroneFrame hasFrame={hasFrame} />
        
        {/* Motors */}
        {motorPositions.slice(0, hasMotors).map((pos, i) => (
          <Motor key={`motor-${i}`} position={pos} color="#00D4FF" />
        ))}
        
        {/* Propellers */}
        {motorPositions.slice(0, Math.min(hasProps, hasMotors)).map((pos, i) => (
          <Propeller key={`prop-${i}`} position={[pos[0], pos[1] + 0.2, pos[2]]} spinning />
        ))}
        
        {/* Battery */}
        {hasBattery && <Battery position={[0, 0.12, 0]} />}
      ...
      >
        <Canvas>
          <Suspense fallback={null}>
            ...
            {/* Drone Model */}
            <DroneModel components={placedComponents} />
            ...
            {placedComponents.length === 0 && (
              <Html center>
                <div className="text-center text-muted-foreground text-sm whitespace-nowrap">
        ...
        {isDragOver && (
          <div className="absolute inset-0 bg-primary/10 flex items-center justify-center pointer-events-none z-10">
            <div className="px-6 py-3 bg-primary/20 border border-primary rounded-lg text-primary font-medium backdrop-blur-sm">
        ...
        {placedComponents.length > 0 && (
          <div className="absolute bottom-4 left-4 px-3 py-1.5 bg-card/80 backdrop-blur-sm border border-border rounded-lg text-xs">
            <span className="text-muted-foreground">Components:</span>{" "}
            <span className="text-primary font-medium">{placedComponents.length}</span>
          </div>
        )}
      </div>
    </div>
